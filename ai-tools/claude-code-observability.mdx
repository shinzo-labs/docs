...
description: "Configure Shinzo observability for Claude Code to track agent conversations, tool usage, token consumption, and performance."
---

# Claude Code Observability

Claude Code is Anthropic's official CLI tool for interacting with Claude directly from your terminal. By routing Claude Code through Shinzo, you gain comprehensive observability into your agent's behavior, including conversation traces, tool usage, token consumption, and performance metrics.

## How It Works

Shinzo integrates with Claude Code to collect observability data automatically:

1. **Automatic Telemetry Collection**: Claude Code has built-in support for sending telemetry to OpenTelemetry-compatible backends
2. **Transparent Proxying**: Your Claude requests are routed through Shinzo for analytics while maintaining full functionality
3. **Rich Context**: Shinzo captures conversation flow, tool calls, MCP server usage, and performance metrics
4. **Dashboard Visualization**: View all your agent activity in the [Shinzo Dashboard](https://app.shinzo.ai)
...
Follow these steps to configure Claude Code to send observability data to Shinzo:
...
## Viewing Your Agent Data

Once configured, you can view your Claude Code observability data in several places:
...
<CardGroup cols={2}>
  ...
  <Card title="Anthropic SDK Observability" icon="code" href="/ai-tools/anthropic-sdk-observability">
    Set up observability for custom agents built with the Anthropic SDK
  </Card>
