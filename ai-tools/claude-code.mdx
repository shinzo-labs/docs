...
<Step title="Verify the configuration">
  ...
  **First-time login flow:**
  ...
  Make a test request and then check the [Shinzo Dashboard](https://app.shinzo.ai) to see your request logged.
...
| Request content | Yes | Your prompts are stored for session analysis |
| Response content | Yes | Claude's responses are stored for session analysis |
...
<Warning>
**Subscription-based access is not supported.** Shinzo requires your own Anthropic API key to proxy requests. OAuth tokens from Claude subscriptions (e.g., Claude Pro/Team subscriptions) cannot be used with Shinzo at this time.

If you attempt to use subscription-based authentication, you will receive a `403 Forbidden` error with the message:
> "Shinzo Platform is currently only compatible with API key-based access. Please add your Anthropic API key in the settings page to continue."
</Warning>

To use Shinzo with Claude Code, you must:

1. Have an Anthropic API key from the [Anthropic Console](https://console.anthropic.com/settings/keys)
2. Store your API key in Shinzo (for "Shinzo as API Key" mode) or keep it in your environment (for "Custom Header" mode)
...
- **403 Forbidden - Subscription not supported**: You are using a Claude subscription OAuth token instead of an API key. Shinzo requires your own Anthropic API key. Get one from the [Anthropic Console](https://console.anthropic.com/settings/keys).
- **403 Forbidden - Invalid key**: Your Anthropic key may be invalid or expired
