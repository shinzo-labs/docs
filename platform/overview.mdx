...
Shinzo Platform provides a seamless observability experience built on industry standards:

```mermaid
graph TB
    A[Your MCP Server] --> B[Shinzo TypeScript SDK]
    B --> C[OpenTelemetry Protocol]
    C --> D[Shinzo Platform]
    D --> E[Web Dashboard]
    E --> F[You - Developer]

    G[OpenTelemetry Collector] --> C
    G --> D
```

### Components

- **TypeScript SDK**: Automatic instrumentation for MCP servers using OpenTelemetry
- **OpenTelemetry Protocol**: Industry-standard telemetry data format
- **Shinzo Platform**: Secure cloud platform for processing and storing observability data
- **Web Dashboard**: Interactive interface for monitoring and analyzing your MCP servers
...
- **Secure Sessions**: Protected web sessions for dashboard access
