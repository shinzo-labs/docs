...
```bash
claude mcp add shinzo --transport http "https://api.app.shinzo.ai/v1/mcp" --header "Authorization: Bearer YOUR_SHINZO_API_KEY"
```
...
For any MCP client that supports HTTP transport (Streamable HTTP or SSE), use these connection details:
...
**Generic `mcpServers` config block:**
...
Once connected to Claude Code, you can ask Claude to create and manage agents conversationally:

```
"Create a new Shinzo agent called 'research-assistant' with a description 
'Helps with research tasks' and a system prompt 'You are a helpful research 
...
```bash
# In Claude Code, Claude will call the create_agent tool:
# Result: Agent "research-assistant" created with ID abc123
```
...
<AccordionGroup>
  <Accordion title="Connection refused or 401 Unauthorized">
    ...
    - Check that the `Authorization` header is formatted as `Bearer YOUR_KEY` (with a space)
  </Accordion>
