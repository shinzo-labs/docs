...
| **Schedules & Triggers** | 11 tools | Automate agents with cron schedules and webhook triggers |
...
| `list_ai_sessions` | List AI interaction sessions with filtering (including `agent_uuid` to scope to a single agent) and pagination |
...
| `update_agent` | Update agent configuration, system prompt, files, MCP server access, or per-agent Discord bot token |
...
### Schedules & Triggers (11 tools)

Automate agent activity with cron schedules and webhook triggers.

#### Schedules (7 tools)

Set up recurring cron jobs that send a message to an agent at a defined cadence.

| Tool | Description |
|------|-------------|
| `create_schedule` | Create a cron schedule for an agent — define expression, timezone, and message template |
| `list_schedules` | List all cron schedules for an agent with pagination |
...
Message templates support variables: `{{date}}`, `{{time}}`, and `{{schedule.name}}`.

#### Triggers (4 tools)

Set up webhook endpoints that send a message to an agent when called.

| Tool | Description |
|------|-------------|
| `create_trigger` | Create a webhook trigger — returns a URL and auto-generated secret. POST to the URL to activate the agent |
| `list_triggers` | List all webhook triggers for an agent |
| `update_trigger` | Update trigger name, message template, or enabled state |
| `delete_trigger` | Delete a webhook trigger |

Message templates support `{{payload.*}}` substitution to embed fields from the incoming webhook JSON body.

**Usage examples:**
- "Create a daily summary schedule for agent abc-123 at 9am UTC on weekdays"
- "Set up a webhook trigger for agent abc-123 to process incoming GitHub events"
- "List all schedules for agent abc-123"
- "Disable the daily report schedule until Monday"
- "Show execution history for schedule xyz-456"
