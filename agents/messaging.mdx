...
The Shinzo agent messaging system lets you send messages to agents and retrieve conversation history. Agents support communication through the **API**, **Discord**, and **Slack** channels. Messages are automatically routed back through the channel they originated from.

<Info>
**Discord** and **Slack** are fully supported channel integrations. Support for additional channels including Telegram and WhatsApp is coming soon.
</Info>
...
Slack is fully supported as an external channel. Linking your agent to Slack allows users to interact with it directly through Slack direct messages via the Shinzo bot.
...
- **Generate link codes close to when they will be used.** Codes expire after 10 minutes for both Discord and Slack, so avoid generating them far in advance.
...
- **Use multiple channels simultaneously.** An agent can maintain active connections to both Discord and Slack at the same time, allowing users to choose their preferred platform.
