...
| `description` | string | No | — | Purpose description, max 500 characters |
| `pvc_size` | string | No | `"1Gi"` | Persistent volume size for agent workspace |
| `anthropic_api_key` | string | No | — | Anthropic API key. If omitted, your stored account key is used |
| `system_prompt` | string | No | — | Custom system prompt for the agent |
| `system_prompt_type` | enum | No | `"append"` | `"replace"` overwrites the default prompt; `"append"` adds to it |
| `configuration` | object | Yes | — | Agent configuration object (see below) |
| `initial_files` | array | No | — | Files to create in the agent workspace on startup |
...
| `model` | string | — | Model name to use for the agent |
...
| `encoding` | enum | No | `"utf-8"` (default) or `"base64"` for binary content |
...
<CardGroup cols={2}>
  ...
  <Card title="Channel Integrations" icon="hashtag" href="/agents/channels">
    Connect agents to Discord and other messaging platforms.
  </Card>
</CardGroup>
